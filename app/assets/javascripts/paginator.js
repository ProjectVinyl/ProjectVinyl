const paginator = (function() {
  function requestPage(context, page) {
    ...
    ajax.get(`${context.attr('data-type')}?page=${context.attr('data-page')}&${context.attr('data-args')}`, json => {
      populatePage(context, json);
    }, {});
  }

  function populatePage(context, json) {
    const container = context.find('ul');
  ...
  if (match = hash.match(/#\/([^\/]+)/)) {
    ...
    if (page > -2) {
      $doc.ready(() => {
        const pagination = $(`.pagination[data-id=${id}]`);
        if (pagination.length) {
          requestPage(pagination.closest('.paginator'), page - 1);
        } else {
          const tab_switch = $(`.tab-set.async a.button[data-target=${id}]`);
