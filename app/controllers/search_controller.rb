class SearchController < ApplicationController
  ...
  def page
    ...
    if params[:query]
      if @type == 1
        @results = Pagination.paginate(Album.where('title LIKE ?', "%#{@query}%"), @page, 45, !@ascending)
        ...
        @results = Pagination.paginate(Artist.where('name LIKE ?', "%#{@query}%"), @page, 45, !@ascending)
        ...
        @results = Pagination.paginate(Video.where('title LIKE ?', "%#{@query}%"), @page, 45, !@ascending)
        render json: {
