...
class Pagination
  ...
  attr_reader :page, :pages, :count, :page_size, :reverse

  def self.paginate(records, page_number, page_size, reverse)
    records = records.reverse_order if reverse
    count = records.size

    return Pagination.new(records, page_size, 0, 0, count, reverse) if count == 0

    page_number = 0 if page_number.nil?

    return Pagination.new(records, page_size, 0, 0, count, reverse) if count <= page_size
    ...
    Pagination.new(records.offset(page_number * page_size).limit(page_size), page_size, pages, page_number, count, reverse)
  end

  def initialize(records, limit, pages, page, count, reverse)
    @page_size = limit
    @offset = page * limit
    @count = count
    @records = records
